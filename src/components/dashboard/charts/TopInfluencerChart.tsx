...
import type { TopInfluencerEntry } from '../../../queries/dashboardData'
import { useTheme } from '../../ThemeProvider'

interface Props {
  data: TopInfluencerEntry[]
}
...
export function TopInfluencerChart({ data }: Props) {
  const { isDark } = useTheme()

  const relevant = data.filter((e) => e.netRevenue > 0).slice(0, 10)
  ...
  return (
    <div className="rounded-md border border-stone-200 bg-stone-100 p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:shadow-none">
      <h3 className="mb-5 text-xl font-semibold text-slate-700 dark:text-gray-300">Top 10 Influencer nach Nettoumsatz</h3>
      {relevant.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={relevant} layout="vertical" margin={{ left: 8 }}>
            ...
            <XAxis type="number" tickFormatter={formatEur} tick={{ fontSize: 12, fill: tickColor }} />
            ...
            />
            <Bar dataKey="netRevenue" name="Nettoumsatz" fill={barColor} radius={[0, 3, 3, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
